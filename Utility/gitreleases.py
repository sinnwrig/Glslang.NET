...
def download_release(release, destination_directory, new_name, compression_type):
    response, downloaded_data = progress_download(release["browser_download_url"], f"Downloading {release['name']}")
...
def download_file(owner, repo, branch, path, destination):
    response, downloaded_data = progress_download(f"https://raw.githubusercontent.com/{owner}/{repo}/{branch}/{path}", f"Downloading file {os.path.basename(os.path.normpath(path))}")
